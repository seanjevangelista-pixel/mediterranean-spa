export default async function handler(req, res) {
  ...
  const time = ts
    ? new Date(ts).toLocaleString('en-US', { timeZone: 'America/New_York', dateStyle: 'full', timeStyle: 'short' })
    : 'Unknown';
  ...
  if (resendKey) {
    const html = `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto">
        <div style="background:#7B5C3A;padding:24px 28px;border-radius:8px 8px 0 0">
          <h2 style="color:#fff;margin:0;font-size:18px">📞 Website Phone Tap</h2>
          <p style="color:rgba(255,255,255,0.65);margin:4px 0 0;font-size:13px">Mediterranean Massage and Esthetics</p>
        </div>
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Mediterranean Massage and Esthetics <bookings@mediterraneanspa.com>',
          to: ['seanjevangelista@gmail.com', 'mediterr20nean@gmail.com'],
